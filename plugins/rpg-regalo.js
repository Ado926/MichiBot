...
const handler = async (m, { conn }) => {
    ...
    if (new Date().getTime() - global.db.data.users[m.sender].lastclaim < tenDaysInMillis) {
        return conn.reply(m.chat, `*Ya Has Reclamado El Regalo De MichiBot💛, Vuelve En ${msToTime(time - new Date().getTime())}*`, m);
    }

    const user = global.db.data.users[m.sender];
    
    conn.sendMessage(m.chat, {text: `🎩 *@${m.sender.split('@')[0]} Michi Te Ha Regalado:*\n> 🌟 500 Estrellas\n> 💶 400 Experiencia\n> 🪙 200 MishiCoins`, mentions: [m.sender]}, {quoted: fkontak});
